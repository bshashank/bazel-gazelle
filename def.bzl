...
def _gazelle_runner_impl(ctx):
    ...
    gazelle_bin = ctx.executable.gazelle.short_path
    if gazelle_bin.startswith('../'):
        # relative paths don't work on Windows
        gazelle_bin = 'external/' + gazelle_bin[3:]

    substitutions = {
        "@@ARGS@@": shell.array_literal(args),
        "@@GAZELLE_LABEL@@": shell.quote(str(ctx.attr.gazelle.label)),
        "@@GAZELLE_PATH@@": shell.quote(ctx.workspace_name + '/' + gazelle_bin)
        "@@GENERATED_MESSAGE@@": """
# Generated by {label}
# DO NOT EDIT
""".format(label = str(ctx.label)),
        "@@RUNNER_LABEL@@": shell.quote(str(ctx.label)),
        "@@GOTOOL@@": shell.quote(ctx.workspace_name + '/' + go_tool.path),
        "@@ENV@@": env,
